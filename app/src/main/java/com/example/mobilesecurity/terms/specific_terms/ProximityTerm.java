...
public class ProximityTerm extends TermLogin implements Orderable {
    private boolean try_again = true;
    public ProximityTerm() {
        super();
    }
    ...
    protected TermStatus isTermOK(TermsListenerInput listenerInput) {
        ...
        Sensor sensorShake = sensorManager.getDefaultSensor(Sensor.TYPE_PROXIMITY);
        SensorEventListener sensorEventListener = new SensorEventListener() {
            @Override
            public void onSensorChanged(SensorEvent sensorEvent) {
                Log.d("mylog", "in light0");
                prox_event_handler(sensorEvent.values[0],this,sensorManager);
    ...
    public String toString() {
        return " Proximity Term";
    }
    private void prox_event_handler(float value, SensorEventListener sensorEventListener, SensorManager sensorManager) {
        if(!try_again)
            return;
